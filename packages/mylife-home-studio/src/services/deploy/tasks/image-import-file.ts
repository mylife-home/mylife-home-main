...
export const metadata: TaskMetadata = {
  description: 'import the specified file into the root fs of the image at the specified path',
  parameters: [
    { name: 'file', description: 'file to import', type: 'string' },
    { name: 'targetPath', description: 'Target path into root fs of image', type: 'string' }
  ],
};

export const execute: TaskImplementation = async (context, parameters) => {
  const { file, targetPath } = parameters;
  const log = createLogger(context, 'image:import-file');
  const fullName = absolutePath(file);
  log.info(`import '${fullName}' into image at '${targetPath}'`);
